...
load("data/Network.RDA", verbose=T)
#load("data/Carabidae_data.RDA", verbose=T)
load("data/Carabidae_variables.RDA", verbose=T)


# load("data/Carabidae_data2.RDA", verbose=T)
# Carabidae_data <- as.data.table(Carabidae_data)
# 
# # # fixing species name, which got messed up ---- ignore me
# # Carabidae_data$Fungus_name <- tax_tab[ fmatch( Carabidae_data$GBIFID_Fungus, tax_tab$GBIFID)]$node_name
# # Carabidae_data$Carabidae_name <- tax_tab[ fmatch( Carabidae_data$GBIFID_Carabidae, tax_tab$GBIFID)]$node_name
# # Carabidae_data[,Fungus_link := ifelse(GBIFID_Fungus == -1, paste0("<i>",Fungus_name,"</i>"), paste0("<a href='https://www.gbif.org/species/", GBIFID_Fungus, "' target='_blank'><i>", Fungus_name, "</i></a>"))]
# # Carabidae_data[,Carabidae_link := ifelse(GBIFID_Carabidae == -1, paste0("<i>",Carabidae_name,"</i>"), paste0("<a href='https://www.gbif.org/species/", GBIFID_Carabidae, "' target='_blank'><i>", Carabidae_name, "</i></a>"))]
# 
# # # adding comments to establish sorting order --- UPDATE
# Carabidae_data[, Carabidae_link := paste0("<!--", Carabidae_name, "-->", Carabidae_link)]
# Carabidae_data[, Fungus_link := paste0("<!--", Fungus_name, "-->", Fungus_link)]
# Carabidae_data[,GBIFID_Carabidae := as.integer(GBIFID_Carabidae)]
# Carabidae_data[,GBIFID_Fungus := as.integer(GBIFID_Fungus)]
# Carabidae_data[,Citation := paste0(  
#     "<!--",References,
#     "--><a id='",
#     lineID,
#     "' href='#' class='action-button' style='color: #000000'>",
#     References,
#     "</a>\n<script>$(document).ready(function() {setTimeout(function() {shinyBS.addTooltip('",
#     lineID,
#     "', 'popover', {'container': 'body', 'content': '",
#     "<MISSING>",
#     "', 'placement': 'left', 'trigger': 'focus', 'title': '<b>",
#     References,
#     "</b>'})}, 500)});</script>")]
# 
# Carabidae_data[,c('lineID'):=NULL]
# 
# tax_tab <- as.data.table(tax_tab)
# tax_tab[,node_name := tax_tab[[tolower(rank)]][.I], by=rank]
# tax_tab[, pathString := gsub("/NA","", pathString, fixed=T)]
# tax_tab[,GBIFID := as.integer(GBIFID)]
# 
# save(Carabidae_data, shp, tax_tab, file="data/Carabidae_data2_mod.RDA")

load("data/Carabidae_data2_mod.RDA", verbose=T)

#### proof of functionality:

# # interaction menu and network legend are dependent on input data
# Carabidae_data <- Carabidae_data[Inter_type %chin% c("ectoparacitic","parasitic","entomopathogenic")]
...
# 
# Carabidae_network_Full <- tidygraph::tbl_graph(nodes = nodes, edges = edges, directed = FALSE, node_key = "name")
# 
# rm(edges, nodes)
#
# save(Carabidae_network_Full, file="Network.RDA")
...
server <- function(input, output, session) {
    ...
    output$NetworkLegendDpdn <- renderUI({
        ...
        if (is.null(EdgeTab$data)) {
            ...
            if (nrow(InAcCodeTab)>8) {tableText$names[8] <- "various"}
    ...
    output$exportTabXLSX <- downloadHandler(
        ...
        content = function(file) {
            updateProgressBar(session, id="ProgBar",  value=0)
            
            shinyjs::toggleElement("ProgBar_div")
            ...
            updateProgressBar(session, id="ProgBar",  value=10)
            ...
            text2 <- paste0("Pozsgai et al., 2021\n",
                            "The association of ground beetles (Coleoptera: Carabidae) with fungi compiled from a global 200-year review\n",
                            "<Journal><vol>(<issue>): <page>.\n", "doi: ...")
            
            updateProgressBar(session, id="ProgBar",  value=15)
            # r2excel::xlsx.addParagraph(workbook, sheet1,
            #                            value=text2,
            #                            isItalic=F,
            #                            colSpan=4,
            #                            startRow = 1, #  specifying it avoids looking it up ...
            #                            rowSpan=6,
            #                            fontColor="#353535",
            #                            fontSize=13)
            sheetParagraph <- xlsx::createCell(xlsx::createRow(sheet1, rowIndex = 1), colIndex = 2)
            xlsx::setCellValue(sheetParagraph[[1, 1]], text2) 
            PARAGRAPH_STYLE <- CellStyle(workbook) + 
                xlsx::Font(workbook, heightInPoints = 13, color = "#353535", isBold = FALSE, isItalic = FALSE) + 
                xlsx::Alignment(wrapText = TRUE, horizontal = "ALIGN_JUSTIFY", vertical = "VERTICAL_CENTER")
            xlsx::setCellStyle(sheetParagraph[[1, 1]], PARAGRAPH_STYLE)
            xlsx::addMergedRegion(sheet1, startRow=1, endRow = 6, startColumn=2, endColumn = 5)
            # cellRangeAddress <- rJava::.jnew("org/apache/poi/ss/util/CellRangeAddress", as.integer(0), as.integer(5), as.integer(1), as.integer(4))
            # ind <- rJava::.jcall(sheet1, "I", "addMergedRegion", cellRangeAddress)
            # xlsx.addLineBreak(sheet1, 6)
            #for (startRow in 2:7) {
                sheetLineBreak <- xlsx::createCell(xlsx::createRow(sheet1, rowIndex = 2:7), colIndex = 1)
                setCellValue(sheetLineBreak[[1, 1]], "  ")
                setCellValue(sheetLineBreak[[2, 1]], "  ")
                setCellValue(sheetLineBreak[[3, 1]], "  ")
                setCellValue(sheetLineBreak[[4, 1]], "  ")
                setCellValue(sheetLineBreak[[5, 1]], "  ")
                setCellValue(sheetLineBreak[[6, 1]], "  ")
            #}
            
            updateProgressBar(session, id="ProgBar",  value=30)
            ...
            #                                colSpan = 1,
            #                                rowSpan = 1,
            #                                fontColor="#010101", # at the time of implementation: white and black are inverted. An 'almost black' might be more resistant to time ...
            #                                fontSize=13)
            # }
            sheetParagraph <- xlsx::createCell(xlsx::createRow(sheet1, rowIndex = 8), colIndex = 1)
            xlsx::setCellValue(sheetParagraph[[1, 1]], "Selected Data:")
            PARAGRAPH_STYLE <- xlsx::CellStyle(workbook) + 
                xlsx::Font(workbook, heightInPoints = 13, color = "#010101", isBold = TRUE, isItalic = FALSE) + 
                xlsx::Alignment(wrapText = TRUE, horizontal = "ALIGN_JUSTIFY", vertical = "VERTICAL_CENTER")
            xlsx::setCellStyle(sheetParagraph[[1, 1]], PARAGRAPH_STYLE)
            
            xlsx::addMergedRegion(sheet1, startRow=8, endRow = 8, startCol=1, endColumn = 1)
            # cellRangeAddress <- rJava::.jnew("org/apache/poi/ss/util/CellRangeAddress", as.integer(7), as.integer(7), as.integer(0), as.integer(0))
            # ind <- rJava::.jcall(sheet1, "I", "addMergedRegion", cellRangeAddress)
            #xlsx.addLineBreak(sheet, 1)
            sheetLineBreak <- xlsx::createCell(xlsx::createRow(sheet1, rowIndex = 9), colIndex = 1)
            setCellValue(sheetLineBreak[[1, 1]], "  ")
            
            updateProgressBar(session, id="ProgBar",  value=50)
            ...
            updateProgressBar(session, id="ProgBar",  value=100)
            shinyjs::toggleElement("ProgBar_div")
    ...
    Carabidae_network <- reactive({
        if (nrow(dataFilter())>0) {
            ...
            SubNetwork <- to_subgraph(Carabidae_network_Full,
                                      name %in% c(dataFilter()$GBIFID_Carabidae, dataFilter()$GBIFID_Fungus),
                                      subset_by = "nodes")$subgraph
            
            EdgeTab$data <- SubNetwork %>% activate(edges) %>% as_tibble()
            return(SubNetwork)
        }else{
            return(NULL)
        }
    ...
    output$SunburstPolt <- renderPlotly({ # c("kingdom","phylum","order","family","genus","species")
        ...
        tableSelection <- dataFilter()[grep(ReactVal$CountryClickSelection, Country),]
        
        if (nrow(tableSelection)>0) {
            
            tableSelection <- tax_tab[
                fastmatch::fmatch(x=c(tableSelection$GBIFID_Carabidae, tableSelection$GBIFID_Fungus),
                                  table=GBIFID),
            ...
            id1 <- is.na(tableSelection_Fungi$phylum)
            tableSelection_Fungi$phylum[id1] <- tableSelection_Fungi$kingdom[id1]
            id2 <- is.na(tableSelection_Fungi$order)
            tableSelection_Fungi$order[id2] <- tableSelection_Fungi$phylum[id2]
            id3 <- is.na(tableSelection_Fungi$family)
            tableSelection_Fungi$family[id3] <- tableSelection_Fungi$order[id3]
            id4 <- is.na(tableSelection_Fungi$genus)
            tableSelection_Fungi$genus[id4] <- tableSelection_Fungi$family[id4] # basically: [genus := ifelse(is.na(genus), family, genus)]
            id5 <- is.na(tableSelection_Carabidae$family)
            tableSelection_Carabidae$family[id5] <- tableSelection_Carabidae$kingdom[id5]
            id6 <- is.na(tableSelection_Carabidae$genus)
            tableSelection_Carabidae$genus[id6] <- tableSelection_Carabidae$family[id6]
            rm(id1,id2,id3,id4,id5,id6)
    ...
    # tree / tree-tab
    output$treeFungus <- renderDiagonalNetwork({ # renderRadialNetwork({
        # ReactVal$CountryClickSelection has the value '*' as default
        ...
        tableSelection <- tax_tab[GBIFID %in% dataFilter()[grep(pattern=ReactVal$CountryClickSelection, x=Country), ]$GBIFID_Fungus, c("GBIFID","pathString")]
        if (nrow(tableSelection)>0) {
            ...
            htmlwidgets::onRender(dN, "function(el, x) { d3.selectAll('.node').on('mouseover', null); }")
            return(dN)
        }else{
            return(NULL)
        }
    ...
    # tree / tree-tab
    output$treeCarabidae <- renderDiagonalNetwork({
        # ReactVal$CountryClickSelection has the value '*' as default
        # we don't need to worry about missing GBIF IDs since we start to work with the taxonomy table (we don't need to count occurrences)
        tableSelection <- tax_tab[GBIFID %in% dataFilter()[grep(pattern=ReactVal$CountryClickSelection, x=Country), ]$GBIFID_Carabidae, c("GBIFID","pathString")]
        if (nrow(tableSelection)) {
